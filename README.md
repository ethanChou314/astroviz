# astroviz
The Python module, **astroviz**, comprehensive tool for data analysis and visualization in radio astronomy. This module simplifies the complexities of handling astronomical data by providing intuitive interfaces and functionalities. It has been tested specifically for CASA/FITS files of ALMA data generated by CASA/CARTA but may read data of other wavelengths as well (not guaranteed).

**Key Features**
* **Data Handling**: Read CASA/FITS files (data cubes, continuum/moment maps, PV diagrams, and two-variable datasets) and their headers. Read DS9 region files that can be used for specific functions.
* **Analysis Tools**: Extract moment maps, PV diagrams, and spectral profiles from data cubes. Includes tools for Gaussian fitting (both 1D and 2D), image rotation and shifting (with multi-processing supported!), regridding, convolution, flux estimation, searching molecular line information, column density calculation (for linear molecules), RMS noise level estimation via sigma clipping, and more.
* **Visualization Tools**: Visualize data cubes, continuum/moment maps, PV diagrams, and two-variable data sets (e.g., spectral profiles). Supports multi-panel figures.
* **High-level Interface**: Supports built-in Python operators for quick math calculations on images. Supports astropy units, enhancing functionality with features like automatic plot label adjustment based on the intensity and axis units of data. Designed for easy customization and extension to fit specific research needs.

**Core Components**
* **Datacube**: Read and analyze three-dimensional radio astronomy data
* **Spatialmap**: Read and analyze two-dimensional spatial data (continuum/moment maps)
* **PVdiagram**: Read and analyze position-velocity (PV) data
* **Plot2D**: Read and analyze two-variable datasets (e.g., spectral profiles)
* **ImageMatrix**: Create publication-quality, multi-panel figures. Includes high-level tools for adding annotations.
* **Region**: Define and utilize regions within data for targeted analysis
...
* other standard Python modules (copy, datetime, os, sys, concurrent, string, inspect, warnings, itertools, typing)
