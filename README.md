# astroviz
The Python module, **astroviz**, is a comprehensive tool for data analysis and visualization in radio astronomy, including functionalities for importing FITS files; handling and manipulating data cubes, spatial maps, and position-velocity (PV) diagrams; and intuitive operations on image objects. It has been tested specifically for FITS files of ALMA data (those generated by CASA/CARTA) and may read data of other wavelengths (not guaranteed). It is designed to be intuitive-to-use and flexible.
...
* other modules in the Python standard library (copy, datetime os, sys, string) 
